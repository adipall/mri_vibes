...
%             end
            beta_12 = ul(:,i)'*B*ur(:,j);
            beta_21 = ul(:,j)'*B*ur(:,i);
            % Right Eigenvector Correction
            if(abs(beta_11) < eps)
                if (abs(beta_12)>eps)
                    urn(:,j) = ur(:,j); %-beta_12/eps*ur(:,i);
                end
            elseif(abs(beta_12)>abs(beta_11)*eps)
            % if(abs(beta_12)>abs(beta_11)*eps)
                urn(:,j) = ur(:,j) - beta_12/beta_11*ur(:,i);
                % disp('Fix Right Vector');
                % [i j]
            end
            % Left Eigenvector Correction
            if(abs(beta_11) < eps)
                if (abs(beta_21)>eps)
                    uln(:,j) = ul(:,j);% - beta_21/eps*ul(:,i);
                end
            elseif(abs(beta_21)>abs(beta_11)*eps)
